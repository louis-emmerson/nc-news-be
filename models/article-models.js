...
function fetchAllArticles(
  ...
  if (
    !allowedOrderQueries.includes(order) ||
    !allowedSortByQueries.includes(sort_by)
  ) {
    return Promise.reject({ status: 400, msg: "Bad Request" })
  } else {
    if (topic !== undefined) {
...
function addNewArticle(newArticleObj) {
  ...
  return db.query(queryString, valueInserts).then((result) => {
    return result.rows[0].article_id
  })
}

function deleteArticleByIDsad(article_id) {
  return deleteCommentByArticleID(article_id).then(() => {
    return db.query(
